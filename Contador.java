/*  */

import java.util.Scanner;

public class Contador {
	
	public static void main(String[] args) {
		Scanner terminal = new Scanner(System.in);

		System.out.println("Digite o primeiro parâmetro: ");
		int parametroUm = terminal.nextInt();

		System.out.println("Digite o segundo parâmetro: ");
		int parametroDois = terminal.nextInt();
		
		try {
			// Chamando o método contendo a lógica de contagem
			contar(parametroUm, parametroDois);
		} catch (ParametrosInvalidosException parmsInvalidosExcpt) {
			// Imprimir a mensagem: O segundo parâmetro deve ser maior que o primeiro
			System.out.println("O parâmetro 'Um' deve ser menor que o parâmetro 'Dois'");
			parmsInvalidosExcpt.printStackTrace();		// Para auxiliar a rastrear o erro
		}

		terminal.close();
	}

	static void contar(int paramUm, int paramDois ) throws ParametrosInvalidosException {
		// Validar se paramUm é MAIOR que paramDois e lançar a exceção
		if (paramUm > paramDois) {
			throw new ParametrosInvalidosException();
		}
		
		int contagem = paramDois - paramUm;

		// Realizar o for para imprimir os números com base na variável contagem
		for (int i = 1; i <= contagem; i++) {
			System.out.println("Imprimindo o número " + i);
		}
	}
	
}

















